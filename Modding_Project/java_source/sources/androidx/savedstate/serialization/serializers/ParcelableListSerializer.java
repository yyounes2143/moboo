...
public final class ParcelableListSerializer implements KSerializer<List<? extends Parcelable>> {
    ...
    public List<Parcelable> deserialize(@NotNull Decoder decoder) {
        if (decoder instanceof SavedStateDecoder) {
            ...
            return SavedStateReader.m105getParcelableListimpl(SavedStateReader.m53constructorimpl(savedStateDecoder.getSavedState$savedstate_release()), savedStateDecoder.getKey$savedstate_release(), Reflection.getOrCreateKotlinClass(Parcelable.class));
        }
    ...
    public void serialize(@NotNull Encoder encoder, @NotNull List<? extends Parcelable> list) {
        if (encoder instanceof SavedStateEncoder) {
            SavedStateEncoder savedStateEncoder = (SavedStateEncoder) encoder;
            SavedStateWriter.m165putParcelableListimpl(SavedStateWriter.m139constructorimpl(savedStateEncoder.getSavedState$savedstate_release()), savedStateEncoder.getKey$savedstate_release(), list);
            return;
        }
