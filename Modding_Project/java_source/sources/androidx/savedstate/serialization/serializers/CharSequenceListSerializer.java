...
public final class CharSequenceListSerializer implements KSerializer<List<? extends CharSequence>> {
    ...
    public List<CharSequence> deserialize(@NotNull Decoder decoder) {
        if (decoder instanceof SavedStateDecoder) {
            ...
            return SavedStateReader.m73getCharSequenceListimpl(SavedStateReader.m53constructorimpl(savedStateDecoder.getSavedState$savedstate_release()), savedStateDecoder.getKey$savedstate_release());
        }
    ...
    public void serialize(@NotNull Encoder encoder, @NotNull List<? extends CharSequence> list) {
        if (encoder instanceof SavedStateEncoder) {
            SavedStateEncoder savedStateEncoder = (SavedStateEncoder) encoder;
            SavedStateWriter.m151putCharSequenceListimpl(SavedStateWriter.m139constructorimpl(savedStateEncoder.getSavedState$savedstate_release()), savedStateEncoder.getKey$savedstate_release(), list);
            return;
        }
