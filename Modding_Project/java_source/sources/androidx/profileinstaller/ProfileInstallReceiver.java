    ...
    public void onReceive(@NonNull Context context, @Nullable Intent intent) {
        ...
        if (intent != null) {
            ...
            if (ACTION_INSTALL_PROFILE.equals(action)) {
                ProfileInstaller.writeProfile(context, new androidx.media3.exoplayer.dash.offline.Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww(), new ResultDiagnostics(), true);
            } else if (ACTION_SKIP_FILE.equals(action)) {
                Bundle extras2 = intent.getExtras();
                if (extras2 != null) {
                    String string = extras2.getString(EXTRA_SKIP_FILE_OPERATION);
                    if (EXTRA_SKIP_FILE_OPERATION_WRITE.equals(string)) {
                        ProfileInstaller.writeSkipFile(context, new androidx.media3.exoplayer.dash.offline.Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww(), new ResultDiagnostics());
                    } else if (EXTRA_SKIP_FILE_OPERATION_DELETE.equals(string)) {
                        ProfileInstaller.deleteSkipFile(context, new androidx.media3.exoplayer.dash.offline.Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww(), new ResultDiagnostics());
                    }
                }
